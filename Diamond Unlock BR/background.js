  ...
  if (msg && msg.action === "lovableApiFetch") {
    (async () => {
      try {
        ...
        const mode = st.sp_lovable_mode || (st.sp_disable_lovable_api ? 'disabled' : 'real');
  ...
  if (msg && msg.action === "lovableApiPutFile") {
    (async () => {
      try {
        ...
        const mode2 = st2.sp_lovable_mode || (st2.sp_disable_lovable_api ? 'disabled' : 'real');
    ...
    (async () => {
      try {
        if (msg && msg.url && /lovable/.test(String(msg.url))) {
          ...
          const mode3 = st3.sp_lovable_mode || (st3.sp_disable_lovable_api ? 'disabled' : 'real');
